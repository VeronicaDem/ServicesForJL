...
public class Graph {
    ...
    public Node<Atom> emptyWordVertex;
    public Node<Atom> getNode(String name) {
        if(containsVertex(name)) {
            return vertexList.get(nameToIndex.get(name));
        }
        return null;
    }
    public Graph() {
     ...
     * @param data
     * @return Node<Atom> новый узел
     */
    public Node<Atom> addVertex(Atom data, String name, boolean needToBeIncludedToEmptyWord) {
     ...
     */
    public Node<Atom> addVertex(Atom data, String name, int level) {
        Node<Atom> newNode = new Node<Atom>(data, false, name);
        vertexList.add(newNode);
        int index = vertexList.size() - 1;
        nameToIndex.put(name, index);
